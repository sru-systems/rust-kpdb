...
mod tests {
    ...
    fn test_from_both_returns_correct_instance() {
        let array = [184, 53, 98, 70, 154, 211, 44, 121, 45, 59, 104, 22, 210, 47, 92, 167, 10,
                     193, 98, 121, 81, 174, 1, 128, 96, 122, 3, 12, 5, 33, 202, 40];
        let key = KeyFile {
    ...
    fn test_from_key_file_returns_correct_instance() {
        let array = [94, 136, 72, 152, 218, 40, 4, 113, 81, 208, 229, 111, 141, 198, 41, 39, 115,
                     96, 61, 13, 106, 171, 189, 214, 42, 17, 239, 114, 29, 21, 66, 216];
        let key = KeyFile {
            key: SecStr::new(vec![0x70, 0x61, 0x73, 0x73, 0x77, 0x6f, 0x72, 0x64]),
            file_type: KeyFileType::Xml,
        };
    ...
    fn test_from_password_returns_correct_instance() {
        let array = [56, 129, 33, 157, 8, 125, 217, 198, 52, 55, 63, 211, 61, 250, 51, 162, 203,
                     107, 252, 108, 82, 11, 100, 184, 187, 96, 239, 44, 235, 83, 74, 231];
        let password = "secret";
        let expected = CompositeKey::secure(array);
